...
export class DrizzleDatabase {
  ...
  private async _findMany(table: Table<any>, options: QueryOptions = {}) {
    ...
    const rows = statement
      .all(...params)
      .map((row: any) => mapRow(table, row)) as Record<string, unknown>[]
...
export function drizzle(sqlite: Database, { schema }: { schema: Record<string, any> }) {
  return new DrizzleDatabase(sqlite, schema as Record<string, Table<any>>)
}
